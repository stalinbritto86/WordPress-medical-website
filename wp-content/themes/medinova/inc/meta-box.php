...
function generate_do_layout_meta_box( $post ) {
	...
	$tabs = apply_filters(
		...
		array(
			'sidebars' => array(
				'title' => esc_html__( 'Sidebars', 'medinova' ),
				'target' => '#generate-layout-sidebars',
				'class' => 'current',
			),
			'footer_widgets' => array(
				'title' => esc_html__( 'Footer Widgets', 'medinova' ),
				'target' => '#generate-layout-footer-widgets',
				'class' => '',
			),
			'disable_elements' => array(
				'title' => esc_html__( 'Disable Elements', 'medinova' ),
				'target' => '#generate-layout-disable-elements',
				'class' => '',
			),
			'container' => array(
				'title' => esc_html__( 'Content Container', 'medinova' ),
		...
		<div class="generate-meta-box-content">
			<div id="generate-layout-sidebars">
				<div class="generate_layouts">
					<label for="generate-sidebar-layout" class="generate-layout-metabox-section-title"><?php esc_html_e( 'Sidebar Layout', 'medinova' ); ?></label>

					<select name="_generate-sidebar-layout-meta" id="generate-sidebar-layout">
						<option value="" <?php selected( $stored_meta['_generate-sidebar-layout-meta'][0], '' ); ?>><?php esc_html_e( 'Default', 'medinova' ); ?></option>
						<option value="right-sidebar" <?php selected( $stored_meta['_generate-sidebar-layout-meta'][0], 'right-sidebar' ); ?>><?php esc_html_e( 'Right Sidebar', 'medinova' ); ?></option>
						<option value="left-sidebar" <?php selected( $stored_meta['_generate-sidebar-layout-meta'][0], 'left-sidebar' ); ?>><?php esc_html_e( 'Left Sidebar', 'medinova' ); ?></option>
						<option value="no-sidebar" <?php selected( $stored_meta['_generate-sidebar-layout-meta'][0], 'no-sidebar' ); ?>><?php esc_html_e( 'No Sidebars', 'medinova' ); ?></option>
						<option value="both-sidebars" <?php selected( $stored_meta['_generate-sidebar-layout-meta'][0], 'both-sidebars' ); ?>><?php esc_html_e( 'Both Sidebars', 'medinova' ); ?></option>
						<option value="both-left" <?php selected( $stored_meta['_generate-sidebar-layout-meta'][0], 'both-left' ); ?>><?php esc_html_e( 'Both Sidebars on Left', 'medinova' ); ?></option>
						<option value="both-right" <?php selected( $stored_meta['_generate-sidebar-layout-meta'][0], 'both-right' ); ?>><?php esc_html_e( 'Both Sidebars on Right', 'medinova' ); ?></option>
					</select>
				</div>
			</div>
			...
			<div id="generate-layout-footer-widgets" style="display: none;">
				<div class="generate_footer_widget">
					<label for="generate-footer-widget" class="generate-layout-metabox-section-title"><?php esc_html_e( 'Footer Widgets', 'medinova' ); ?></label>

					<select name="_generate-footer-widget-meta" id="generate-footer-widget">
						<option value="" <?php selected( $stored_meta['_generate-footer-widget-meta'][0], '' ); ?>><?php esc_html_e( 'Default', 'medinova' ); ?></option>
						<option value="0" <?php selected( $stored_meta['_generate-footer-widget-meta'][0], '0' ); ?>><?php esc_html_e( '0 Widgets', 'medinova' ); ?></option>
						<option value="1" <?php selected( $stored_meta['_generate-footer-widget-meta'][0], '1' ); ?>><?php esc_html_e( '1 Widgets', 'medinova' ); ?></option>
						<option value="2" <?php selected( $stored_meta['_generate-footer-widget-meta'][0], '2' ); ?>><?php esc_html_e( '2 Widgets', 'medinova' ); ?></option>
						<option value="3" <?php selected( $stored_meta['_generate-footer-widget-meta'][0], '3' ); ?>><?php esc_html_e( '3 Widgets', 'medinova' ); ?></option>
						<option value="4" <?php selected( $stored_meta['_generate-footer-widget-meta'][0], '4' ); ?>><?php esc_html_e( '4 Widgets', 'medinova' ); ?></option>
						<option value="5" <?php selected( $stored_meta['_generate-footer-widget-meta'][0], '5' ); ?>><?php esc_html_e( '5 Widgets', 'medinova' ); ?></option>
					</select>
				</div>
			</div>
			<div id="generate-layout-page-builder-container" style="display: none;">
				<label for="_generate-full-width-content" class="generate-layout-metabox-section-title"><?php esc_html_e( 'Content Container', 'medinova' ); ?></label>

				<p class="page-builder-content" style="color:#666;font-size:13px;margin-top:0;">
					<?php esc_html_e( 'Choose your content container type.', 'medinova' ); ?>
				</p>

				<select name="_generate-full-width-content" id="_generate-full-width-content">
					<option value="" <?php selected( $stored_meta['_generate-full-width-content'][0], '' ); ?>><?php esc_html_e( 'Default', 'medinova' ); ?></option>
					<option value="true" <?php selected( $stored_meta['_generate-full-width-content'][0], 'true' ); ?>><?php esc_html_e( 'Full Width', 'medinova' ); ?></option>
					<option value="contained" <?php selected( $stored_meta['_generate-full-width-content'][0], 'contained' ); ?>><?php esc_html_e( 'Contained', 'medinova' ); ?></option>
				</select>
			</div>
			<div id="generate-layout-disable-elements" style="display: none;">
				<label class="generate-layout-metabox-section-title"><?php esc_html_e( 'Disable Elements', 'medinova' ); ?></label>
				<?php if ( ! defined( 'GENERATE_DE_VERSION' ) ) : ?>
					<div class="generate_disable_elements">
						<label for="meta-generate-disable-headline" style="display:block;margin: 0 0 1em;" title="<?php esc_attr_e( 'Content Title', 'medinova' ); ?>">
							...
							<?php esc_html_e( 'Content Title', 'medinova' ); ?>
						</label>

						<?php if ( ! defined( 'GP_PREMIUM_VERSION' ) ) : ?>
							<span style="display:block;padding-top:1em;border-top:1px solid #EFEFEF;">
								<a href="<?php echo generate_get_premium_url( 'https://medinova.com/downloads/generate-disable-elements' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function. ?>" target="_blank"><?php esc_html_e( 'Premium module available', 'medinova' ); ?></a>
							</span>
						<?php endif; ?>
					</div>
				<?php endif; ?>
