<?php
/**
 * Integrate medinova with the WordPress block editor.
 *
 * @package medinova
 */
...
function generate_add_inline_block_editor_styles( $editor_settings ) {
	...
	if ( $show_editor_styles ) {
		if ( generate_is_using_dynamic_typography() ) {
			$google_fonts_uri = medinova_Typography::get_google_fonts_uri();
...
function generate_enqueue_backend_block_editor_assets() {
	...
	if ( $show_editor_styles ) {
		// Using wp-edit-blocks for now until we do this: https://github.com/tomusborne/medinova/pull/343.
		wp_add_inline_style( 'wp-edit-blocks', wp_strip_all_tags( generate_do_inline_block_editor_css() ) );

		if ( generate_is_using_dynamic_typography() ) {
			wp_add_inline_style( 'wp-edit-blocks', wp_strip_all_tags( medinova_Typography::get_css( 'core', 'editor' ) ) );
		}
	}
	...
	wp_localize_script(
		...
		array(
			...
			'disable_content_title' => esc_html__( 'Disable Content Title', 'medinova' ),
			'show_content_title' => esc_html__( 'Show Content Title', 'medinova' ),
