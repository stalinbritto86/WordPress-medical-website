...
if ( ! function_exists( 'generate_content_nav' ) ) {
	...
	function generate_content_nav( $nav_id ) {
		...
		<nav <?php generate_do_attr( 'post-navigation', array( 'id' => esc_attr( $nav_id ) ) ); ?>>
			<span class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'medinova' ); ?></span>
			...
			if ( is_single() ) : // navigation links for single posts.
				...
				$post_navigation_args = apply_filters(
					...
					array(
						'previous_format' => '<div class="nav-previous">' . generate_get_svg_icon( 'arrow-left' ) . '<span class="prev" title="' . esc_attr__( 'Previous', 'medinova' ) . '">%link</span></div>',
						'next_format' => '<div class="nav-next">' . generate_get_svg_icon( 'arrow-right' ) . '<span class="next" title="' . esc_attr__( 'Next', 'medinova' ) . '">%link</span></div>',
						'link' => '%title',
			...
			elseif ( is_home() || is_archive() || is_search() ) : // navigation links for home, archive, and search pages.
				...
				if ( get_next_posts_link() ) :
					...
					<div class="nav-previous">
						...
						<span class="prev" title="<?php esc_attr_e( 'Previous', 'medinova' ); ?>"><?php next_posts_link( __( 'Older posts', 'medinova' ) ); ?></span>
					</div>
					<?php
				endif;
				...
				if ( get_previous_posts_link() ) :
					...
					<div class="nav-next">
						<?php generate_do_svg_icon( 'arrow' ); ?>
						<span class="next" title="<?php esc_attr_e( 'Next', 'medinova' ); ?>"><?php previous_posts_link( __( 'Newer posts', 'medinova' ) ); ?></span>
					</div>
					<?php
				endif;
					...
					the_posts_pagination(
						array(
							...
							'before_page_number' => sprintf(
								...
								_x( 'Page', 'prepends the pagination page number for screen readers', 'medinova' )
							),
						)
					);
				}
...
function generate_do_post_meta_item( $item ) {
	...
	if ( 'author' === $item ) {
		...
		echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			...
			sprintf(
				...
				esc_attr( sprintf( __( 'View all posts by %s', 'medinova' ), get_the_author() ) ),
	...
	if ( 'categories' === $item ) {
		$term_separator = apply_filters( 'generate_term_separator', _x( ', ', 'Used between list items, there is a space after the comma.', 'medinova' ), 'categories' );
		...
		if ( $categories_list ) {
			echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				sprintf(
					...
					esc_html_x( 'Categories', 'Used before category names.', 'medinova' ),
					$categories_list,
	...
	if ( 'tags' === $item ) {
		$term_separator = apply_filters( 'generate_term_separator', _x( ', ', 'Used between list items, there is a space after the comma.', 'medinova' ), 'tags' );
		...
		if ( $tags_list ) {
			echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				sprintf(
					...
					esc_html_x( 'Tags', 'Used before tag names.', 'medinova' ),
					$tags_list,
	...
	if ( 'comments-link' === $item ) {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
				...
				comments_popup_link( __( 'Leave a comment', 'medinova' ), __( '1 Comment', 'medinova' ), __( '% Comments', 'medinova' ) );
			echo '</span> ';
		}
	}
	...
	function generate_excerpt_more( $more ) {
		return apply_filters(
			...
			sprintf(
				...
				__( 'Read more', 'medinova' ),
				sprintf(
					/* translators: Aria-label describing the read more button */
					_x( 'More on %s', 'more on post title', 'medinova' ),
					the_title_attribute( 'echo=0' )
				)
			)
		);
