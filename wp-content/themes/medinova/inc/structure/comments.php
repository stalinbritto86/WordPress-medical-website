	...
	function generate_comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div class="comment-body">
				<?php esc_html_e( 'Pingback:', 'medinova' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'medinova' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
			...
			<article <?php generate_do_attr( 'comment-body', array(), array( 'comment-id' => get_comment_ID() ) ); ?>>
				<footer <?php generate_do_attr( 'comment-meta' ); ?>>
					...
					<div class="comment-author-info">
						...
						if ( apply_filters( 'generate_show_comment_entry_meta', true ) ) :
							...
							<div class="entry-meta comment-metadata">
									...
									<time datetime="<?php comment_time( 'c' ); ?>" itemprop="datePublished">
										<?php
											printf(
												...
												_x( '%1$s at %2$s', '1: date, 2: time', 'medinova' ), // phpcs:ignore
								...
								<?php edit_comment_link( __( 'Edit', 'medinova' ), '<span class="edit-link">| ', '</span>' ); ?>
							</div>
							<?php
						endif;
						?>
					...
					<?php if ( '0' == $comment->comment_approved ) : // phpcs:ignore ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'medinova' ); ?></p>
					<?php endif; ?>
				</footer>
...
function generate_set_comment_form_defaults( $defaults ) {
	...
	$defaults['title_reply']          = apply_filters( 'generate_leave_comment', __( 'Leave a Comment', 'medinova' ) );
	$defaults['label_submit']         = apply_filters( 'generate_post_comment', __( 'Post Comment', 'medinova' ) );
