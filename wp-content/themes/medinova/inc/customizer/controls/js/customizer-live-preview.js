 ...
 * @param get_value
 */
function medinova_colors_live_update( id, selector, property, default_value, get_value ) {
...
function medinova_classes_live_update( id, classes, selector, prefix ) {
...
function medinova_typography_live_update( id, selector, property, unit, media, settings ) {
		...
		value.bind( function( newval ) {
			...
			if ( ! isTablet && ! isMobile ) {
				...
				if ( '' !== tabletValue && '' == mobileValue ) {
					media = medinova_live_preview.desktop + ', ' + medinova_live_preview.mobile;
				}

				// The tablet setting doesn't exist, mobile does
				if ( '' == tabletValue && '' !== mobileValue ) {
					media = medinova_live_preview.desktop + ', ' + medinova_live_preview.tablet;
				}

				// The tablet setting doesn't exist, neither does mobile
				if ( '' == tabletValue && '' == mobileValue ) {
					media = medinova_live_preview.desktop + ', ' + medinova_live_preview.tablet + ', ' + medinova_live_preview.mobile;
				}
			}
...
( function( $ ) {
	...
	if ( 'undefined' === typeof gp_premium_typography_live_update ) {
		 ...
		 */
		medinova_typography_live_update( 'body_font_size', 'body, button, input, select, textarea', 'font-size', 'px' );
		medinova_typography_live_update( 'body_line_height', 'body', 'line-height', '' );
		medinova_typography_live_update( 'paragraph_margin', 'p, .entry-content > [class*="wp-block-"]:not(:last-child)', 'margin-bottom', 'em' );
		medinova_typography_live_update( 'body_font_weight', 'body, button, input, select, textarea', 'font-weight' );
		medinova_typography_live_update( 'body_font_transform', 'body, button, input, select, textarea', 'text-transform' );

		/**
		 * H1 font size, weight and transform
		 */
		medinova_typography_live_update( 'heading_1_font_size', 'h1', 'font-size', 'px', medinova_live_preview.desktop );
		medinova_typography_live_update( 'mobile_heading_1_font_size', 'h1', 'font-size', 'px', medinova_live_preview.mobile );
		medinova_typography_live_update( 'heading_1_weight', 'h1', 'font-weight' );
		medinova_typography_live_update( 'heading_1_transform', 'h1', 'text-transform' );
		medinova_typography_live_update( 'heading_1_line_height', 'h1', 'line-height', 'em' );

		/**
		 * H2 font size, weight and transform
		 */
		medinova_typography_live_update( 'heading_2_font_size', 'h2', 'font-size', 'px', medinova_live_preview.desktop );
		medinova_typography_live_update( 'mobile_heading_2_font_size', 'h2', 'font-size', 'px', medinova_live_preview.mobile );
		medinova_typography_live_update( 'heading_2_weight', 'h2', 'font-weight' );
		medinova_typography_live_update( 'heading_2_transform', 'h2', 'text-transform' );
		medinova_typography_live_update( 'heading_2_line_height', 'h2', 'line-height', 'em' );

		/**
		 * H3 font size, weight and transform
		 */
		medinova_typography_live_update( 'heading_3_font_size', 'h3', 'font-size', 'px' );
		medinova_typography_live_update( 'heading_3_weight', 'h3', 'font-weight' );
		medinova_typography_live_update( 'heading_3_transform', 'h3', 'text-transform' );
		medinova_typography_live_update( 'heading_3_line_height', 'h3', 'line-height', 'em' );
	}
	 ...
	 * Top bar alignment
	 */
	medinova_classes_live_update( 'top_bar_alignment', [ 'left', 'center', 'right' ], '.top-bar', 'top-bar-align-' );
	 ...
	 * Header alignment
	 */
	medinova_classes_live_update( 'header_alignment_setting', [ 'left', 'center', 'right' ], 'body', 'header-aligned-' );
		...
		value.bind( function( newval ) {
			...
			if ( $( 'body' ).hasClass( 'sticky-enabled' ) ) {
				...
				if ( 'contained-nav' == newval ) {
					if ( ! mainNavigation.hasClass( 'has-branding' ) && medinova_live_preview.isFlex && ( 'nav-float-right' === navLocation || 'nav-float-left' === navLocation ) ) {
						return;
					}
	...
	wp.customize( 'generate_settings[nav_alignment_setting]', function( value ) {
		value.bind( function( newval ) {
			...
			if ( medinova_live_preview.isFlex ) {
			...
			if ( medinova_live_preview.isFlex && medinova_live_preview.isRTL ) {
				jQuery( selector ).addClass( prefix + newval );
	 ...
	 */
	medinova_classes_live_update( 'footer_bar_alignment', [ 'left', 'center', 'right' ], '.site-footer', 'footer-bar-align-' );
	...
	jQuery( 'body' ).on( 'generate_spacing_updated', function() {
		var containerAlignment = wp.customize( 'generate_settings[container_alignment]' ).get(),
			...
			contentLeft = medinova_live_preview.contentLeft,
			contentRight = medinova_live_preview.contentRight;

		if ( ! medinova_live_preview.isFlex && 'text' === containerAlignment ) {
		...
		if ( medinova_live_preview.isFlex && 'boxes' === containerAlignment ) {
