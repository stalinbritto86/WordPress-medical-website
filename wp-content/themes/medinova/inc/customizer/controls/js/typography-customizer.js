( function( api ) {
	...
	api.controlConstructor['gp-customizer-typography'] = api.Control.extend( {
		ready: function() {
			...
			control.container.on( 'change', '.medinova-font-family select',
				function() {
					...
					setTimeout( function() {
						...
						if ( id in fonts ) {
							...
							jQuery( '.medinova-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.medinova-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.medinova-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.medinova-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.medinova-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.medinova-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.medinova-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.medinova-font-variant' );
			...
			control.container.on( 'change', '.medinova-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.medinova-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.medinova-font-transform select',
				function() {
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.medinova-font-family select' ).select2();

	$( '.medinova-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".medinova-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.medinova-font-variant' ).hide();
		}
	} );
