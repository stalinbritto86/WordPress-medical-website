( function( api ) {
	...
	api( 'generate_header_helper', function( value ) {
		...
		value.bind( function( newval ) {
			...
			var options = {
				headerAlignment: medinova_defaults.header_alignment_setting,
				navLocation: medinova_defaults.nav_position_setting,
				navAlignment: medinova_defaults.nav_alignment_setting,
				boxAlignment: medinova_defaults.container_alignment,
				siteTitleFontSize: medinova_typography_defaults.site_title_font_size,
				hideSiteTagline: medinova_defaults.hide_tagline,
				headerPaddingTop: medinova_spacing_defaults.header_top,
				headerPaddingBottom: medinova_spacing_defaults.header_bottom,
			};
		...
		value.bind( function( newval ) {
			if ( newval ) {
				if ( fontManager.setting.get().length === 0 ) {
					fontManager.setting.set( medinovaCustomizeControls.mappedTypographyData.fonts );
				}

				if ( typographyManager.setting.get().length === 0 ) {
					typographyManager.setting.set( medinovaCustomizeControls.mappedTypographyData.typography );
				}
			}
		} );
	} );
