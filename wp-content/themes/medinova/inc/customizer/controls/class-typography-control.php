...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Generate_Typography_Customize_Control' ) ) {
	...
	class Generate_Typography_Customize_Control extends WP_Customize_Control {
		...
		public function to_json() {
			...
			$this->json['default_fonts_title'] = __( 'System fonts', 'medinova' );
			$this->json['google_fonts_title'] = __( 'Google fonts', 'medinova' );
			$this->json['default_fonts'] = generate_typography_default_fonts();
			$this->json['family_title'] = esc_html__( 'Font family', 'medinova' );
			$this->json['weight_title'] = esc_html__( 'Font weight', 'medinova' );
			$this->json['transform_title'] = esc_html__( 'Text transform', 'medinova' );
			$this->json['category_title'] = '';
			$this->json['variant_title'] = esc_html__( 'Variants', 'medinova' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.family ) ) { #>
				<div class="medinova-font-family">
					<label>
						<select {{{ data.family.link }}} data-category="{{{ data.category.id }}}" data-variants="{{{ data.variant.id }}}" style="width:100%;">
							...
							<optgroup label="{{ data.google_fonts_title }}">
								<# for ( var key in medinovaTypography.googleFonts ) { #>
									<option value="{{ medinovaTypography.googleFonts[ key ].name }}"  <# if ( medinovaTypography.googleFonts[ key ].name === data.family.value ) { #>selected="selected"<# } #>>{{ medinovaTypography.googleFonts[ key ].name }}</option>
								<# } #>
							</optgroup>
						</select>
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				var font_data = medinovaTypography.googleFonts[id];
				var variants = '';
				...
				#>
				<div id={{{ data.variant.id }}}" class="medinova-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="medinova-font-category">
					<label>
			...
			<div class="medinova-weight-transform-wrapper">
				<# if ( 'undefined' !== typeof ( data.weight ) ) { #>
					<div class="medinova-font-weight">
						<label>
				...
				<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
					<div class="medinova-font-transform">
						<label>
