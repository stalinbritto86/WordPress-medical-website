	...
	if ( 'querySelector' in document && 'addEventListener' in window ) {
		...
		var enableDropdownArrows = function( nav ) {
			if ( body.classList.contains( 'dropdown-hover' ) ) {
				...
				for ( i = 0; i < dropdownItems.length; i++ ) {
					...
					dropdownItems[ i ].querySelector( '.dropdown-menu-toggle' ).setAttribute( 'aria-label', medinovaMenu.openSubMenuLabel );
				}
			}
		};
		...
		var setDropdownArrowAttributes = function( arrow ) {
			if ( 'false' === arrow.getAttribute( 'aria-expanded' ) || ! arrow.getAttribute( 'aria-expanded' ) ) {
				...
				arrow.setAttribute( 'aria-label', medinovaMenu.closeSubMenuLabel );
			} else {
				arrow.setAttribute( 'aria-expanded', 'false' );
				arrow.setAttribute( 'aria-label', medinovaMenu.openSubMenuLabel );
			}
		};
		...
		var toggleSubNav = function( e, _this ) {
			...
			if ( ( _this.closest( 'nav' ).classList.contains( 'toggled' ) || htmlEl.classList.contains( 'slide-opened' ) ) && ! body.classList.contains( 'dropdown-click' ) ) {
				...
				if ( medinovaMenu.toggleOpenedSubMenus ) {
